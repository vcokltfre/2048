...
from .cell import CELL_OFFSET, Cell
...
class Game:
    def __init__(self) -> None:
        self._cells: list[list[Cell]] = [[Cell() for _ in range(4)] for _ in range(4)]
    ...
    def draw(self, window: Surface) -> None:
        ...
        text = get_font().render(score_text, True, (0x77, 0x6E, 0x65))
        window.blit(text, (16, CELL_OFFSET * 4 + 8))
    ...
    def _shift_vertical(self, direction: ShiftDirection) -> None:
        ...
        self._cells = [[new_cells[row][col] for row in range(4)] for col in range(4)]
    ...
    def _compact_cells(self, cells: list[Cell]) -> list[Cell]:
        ...
        return [*compacted_cells, *([Cell()] * (4 - len(compacted_cells)))]
